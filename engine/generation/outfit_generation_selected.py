...
from engine.compatibility import invalid_pattern_combo, is_too_similar
...
def generate_outfits_from_selected_garment(
    ...
    # Combinaciones inválidas — bloquear antes de cualquier procesamiento
    ...
    def maybe_add_extras(base: List[Garment], top_item: Optional[Garment] = None):
        ...
        matrimonio_midlayer_allowed = (
            occasion == "matrimonio"
            and mood in ["urbano", "sexy", "comodo", "relajado"]
            and temp >= 24
        )
        if (
    ...
    if selected_category == "top":
        ...
        for top in top_candidates["top"]:
            if occasion == "matrimonio" or (occasion == "cita" and mood == "elegante"):
                if occasion == "matrimonio" and mood == "relajado":
                    if top.dress_level not in ["flexible", "arreglado", "elegante"]:
                        continue
                elif not (occasion == "matrimonio" and mood in ["urbano", "comodo"]):
